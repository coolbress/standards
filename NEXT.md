...
## ⭐ 2026-09-02 — **리뷰 *방식* 의 최신 흐름을 재고, ⓒ 의 내용을 정했다**

**연구와 실무가 같은 곳으로 수렴했다: finding 의 증거는 산문이 아니라 *실행* 이다.**

| | |
|---|---|
| `IPW-021` **c-CRAB** | 리뷰 코멘트를 **실행 가능한 시험**으로 바꿔 오라클로 쓴다. Claude Code **32.1%** · Codex **20.1%** · **넷 합쳐 41.5%** — 🔴 **«사람 100%» 는 경쟁 점수가 아니라 *구성상 천장* 이다**(시험이 바로 그 사람 코멘트에서 만들어졌다). ***«사람이 모델보다 낫다» 로 읽지 마라.*** 🎯 그런데 **코멘트 84%는 유용** — 격차는 *소음* 이 아니라 **우선순위 차이**, 최강 지렛대는 **저장소 고유 맥락** |
| `IPW-022` **과잉교정** | 맞는 코드를 틀렸다 한다. *설명·수정안을 요구할수록 **오판이 는다***. 처방은 **Fix-guided Verification Filter**(수정안을 실행 가능한 반사실 증거로) |

🔬 **우리는 이미 손으로 하고 있었다** — 이 세션에서 고칠 때마다 **변이**를 넣었고(되돌리면 빨개지나) **두 번은 안 물려 가짜 시험을 잡았다.**
**빠진 건 규율이 계약이 아니라는 것뿐**이라, `direction/04` 에 **처분 셋**을 사전등록했다: **ⓐ 고쳤다(되돌리면 빨개지는 시험) · ⓑ 재현 불가(이유) · ⓒ 범위 밖(근거)**.
🔴 **아직 안 만든다** — 표본(**리뷰된 PR 수**)이 `ENOUGH=20` 에 **안 닿았다.** 🔵 **수는 여기 박지 않는다** — `python3 tools/check_review_signal.py` 가 정본이다(전에 이 문서 안에서 **17 과 15 로 갈렸다** · 제3자 지적 2026-09-02). **내용과 재료는 갖춰졌고 표본만 남았다.**

> ⚠️ **긴장을 지우지 않았다**: `IPW-022` 는 설명 요구가 오판을 늘린다는데 **우리 `AGENTS.md` 는 재현 시나리오를 요구한다.**
> 실측(PR당 **6.1** · 대리지표 **80%**)에선 안 보이지만 **가설이지 판정이 아니다.**
> ⚠️ 둘 다 **동료심사 전 프리프린트**이고, `IPW-022` 는 **수치를 하나도 안 썼다**(초록에만 접근했다).

## 🔴 2026-09-02 — **`R5-48` 신설: 리뷰어는 *설치자* 가 고른다**

코덱스가 *usage limits* 로 멈춰 [`#228`](https://github.com/coolbress/standards/pull/228) 이
**머지 불가**가 됐다. 벽은 정확히 동작했다 — **문제는 단일 공급자다.**
🔵 **비용 축이 하나 더 있다** — 특정 벤더를 벽에 박으면 **설치자에게 그 구독을 강제한다.**
이 저장소의 목적 한 줄은 주어가 *외부 설치자* 다.

🔵 **만들 게 거의 없다 — 벽이 이미 벤더 중립이다.** 보증하는 것은 *«인정한 제3자가 이 커밋을
봤다»* 뿐이고 **어느 벤더인지는 안 본다**(`reviewer-logins` 는 목록 · 판정은 집합 검사).
**성질은 있는데 계약으로 안 적혀 있을 뿐이다.**

🔬 **강제는 아직 없다**(실측): `project-template/template/.github/workflows/` 는
`ci.yml`·`label.yml` **둘뿐** — **벽이 템플릿에 안 실려 있다.**
⚠️ **그래서 지금이 정할 때다.** 실을 때 벤더가 박히면 **그때부터 구독 강제**다.

🔴 **반대 위험이 더 크다**: 중립이 *«아무나 통과»* 로 무너지면 벽이 아니다 —
**아무도 안 꽂았을 때 막히는 성질**이 계약의 절반이다.
🔴 **벤더 선택은 가용성·비용을 풀지 품질을 안 푼다** — 품질은 `R5-40` ⓒ 의 **처분 계약**이 푼다.
🚫 **자체 런타임은 안 만든다** · ⚠️ **Claude 액션은 «기각» 이 아니라 «보류»** — *«종량제라서»* 는 기준이 아니고(소유자 지적) **단가를 안 쟀다.** 대장(`R5-48`)이 정본이다.
...
> 갱신 **2026-09-02 (`R5-45`·`R5-46` 을 둘 다 되돌린 뒤)**. 열린 격차 **7**(`check_gaps_ledger` 가 정본 — `R5-48` 이 2026-09-02 에 열렸다).
...
| 3 | `R5-40` ⓒ 다음 층 — **리뷰 처분 계약** | 🟡 **20건 뒤** — 🔵 **다만 재료가 생겼다** | 표본은 `check_review_signal` 이 정본 / `ENOUGH=20`. 🔵 **처분 기록 없이 머지된 지적 18건 · PR 12개**(2026-09-01 신설 계기 `merged_open` · **오탐 0**) — **이 층이 기다리던 것**이다. 🔴 **인용 전에 `check_review_signal` 을 돌려라** |
