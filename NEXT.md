...
| 묻는 것 | 정본 |
|---|---|
| **격차가 몇 건 남았나** | `python3 tools/check_gaps_ledger.py` — 지금 **7건** (⚠️ 이 줄은 **낡는다.** 🔴 **수를 인용하기 전에 명령을 돌려라** — 2026-09-01 에 이 줄이 5 로 남아 제3자에게 잡혔다) |
...
> 갱신 **2026-09-01**. 열린 격차 **7**(`check_gaps_ledger` 가 정본).
> 🔴 **지금 되는 것과 기다려야 하는 것을 갈라 적는다** — 섞으면 기다리는 것을 억지로 만들게 된다.

| # | 할 일 | 지금 되나 | 왜 그 순서 |
|---|---|---|---|
| **1** | **`R5-44` — GitLab 3종 실측**<br>① Developer 토큰이 `push_access_level: 0` 브랜치에 **정말 못 미나** ② 파이프라인 실패가 **머지를 정말 막나** ③ **Free SAST/시크릿이 보고만 하고 끝나나** | 🟡 **계정은 생겼다(2026-09-01) — 아직 못 돈다** | 🔴 **계정 ≠ 자격증명.** 빈 **비공개 프로젝트** + **`api` 스코프 토큰**이 있어야 돈다(소유자만 만든다). ⚠️ **그리고 ① 은 전제부터 확인해야 한다** — *Developer 역할* 로 밀어봐야 하는데 **단독 계정이면 자기가 Owner** 다. **②③ 은 그것과 무관하게 바로 잰다** — 그래서 토큰만 오면 시작할 수 있다. *비공개·외부* 범위를 **실제로** 넓히는 유일한 항목 |
| **2** | **`R5-37` — 수단을 ⓑ 로 바꾼다**(회부 기록을 **PR 본문 표시**로 · 사후 완화가 아니라 **수단 변경**임을 명시) | ✅ 지금 | 회부가 **4건뿐**이라 ⓐ(이슈 코멘트 누적)는 **재료가 안 쌓인다** |
| **3** | **`R5-46` — 심판을 표시한다** | ✅ 지금 | 오늘 만든 **리뷰 지시 핀의 확장**이라 싸다 |
| **4** | **`R5-45` — 유지비 계기** | ✅ 지금 | 🔴 **창립 실패를 재는 유일한 눈금.** 임계값은 **안 긋는다**(`R5-2` 규율) |
| 5 | `R5-40` ⓒ 다음 층 — **리뷰 처분 계약** | 🟡 **20건 뒤** | 계기 **8건** / `ENOUGH=20`. 재판정할 재료가 없다 |
| 6 | `R5-24` — 스킬 발화 | ⏳ **세션이 재료** | 할 일이 아니다. 굴리면 쌓인다 |
| 7 | `R5-26` — 에스컬레이션 3칸 | ⏳ **마찰이 재료** | 상상으로 만들면 **일곱 번째 하네스** |
| 8 | `R5-40` 닫기 | ⏳ `R5-24` 대기 | **ⓒ 는 2026-09-01 에 섰다.** ⓑ 만 남았다 |

### 🚫 지금 만들지 않는다 (제3자 감사와 판단이 일치했다)

Node 상자 · GitLab **추상화 층** · 스킬 확대 · 멀티에이전트 기본값 · 자체 런타임 ·
필수 검사 추가. **실제 수요가 나올 때 만든다.**
⚠️ **`R5-44` 의 GitLab 은 "만들기"가 아니라 "재기"** 라 여기 안 걸린다.
