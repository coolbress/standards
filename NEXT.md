...
| # | 할 일 | 지금 되나 | 왜 그 순서 |
|---|---|---|---|
| **1** | **`R5-44` — GitLab 3종 실측**<br>① Developer 토큰이 `push_access_level: 0` 브랜치에 **정말 못 미나** ② 파이프라인 실패가 **머지를 정말 막나** ③ **Free SAST/시크릿이 보고만 하고 끝나나** | 🟡 **계정은 생겼다(2026-09-01) — 아직 못 돈다** | 🔴 **계정 ≠ 자격증명.** 세 프로브는 **빈 비공개 프로젝트 하나** + **`api` 스코프 토큰** + **Developer 권한 계정 하나**가 있어야 돈다 — 토큰은 **소유자만** 만든다. 그것만 오면 **반나절**이고, *비공개·외부* 범위를 **실제로** 넓히는 유일한 항목 |
