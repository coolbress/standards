...
> 갱신 **2026-09-01 (3차 · GitLab 프로브 뒤)**. 열린 격차 **6**(`check_gaps_ledger` 가 정본).
> 🔴 **지금 되는 것과 기다려야 하는 것을 갈라 적는다** — 섞으면 기다리는 것을 억지로 만들게 된다.

| # | 할 일 | 지금 되나 | 왜 그 순서 |
|---|---|---|---|
| **1** | **`R5-37` — 수단을 ⓑ 로 바꾼다**(회부 기록을 **PR 본문 표시**로 · 사후 완화가 아니라 **수단 변경**임을 명시) | ✅ 지금 | 회부가 **4건뿐**이라 ⓐ(이슈 코멘트 누적)는 **재료가 안 쌓인다** |
| **2** | **`R5-46` — 심판을 표시한다** | ✅ 지금 | 오늘 만든 **리뷰 지시 핀의 확장**이라 싸다 |
| **3** | **`R5-45` — 유지비 계기** | ✅ 지금 | 🔴 **창립 실패를 재는 유일한 눈금.** 임계값은 **안 긋는다**(`R5-2` 규율) |
| 4 | `R5-40` ⓒ 다음 층 — **리뷰 처분 계약** | 🟡 **20건 뒤** | 계기 **8건** / `ENOUGH=20`. 재판정할 재료가 없다 |
| 5 | `R5-24` — 스킬 발화 | ⏳ **세션이 재료** | 할 일이 아니다. 굴리면 쌓인다 |
| 6 | `R5-26` — 에스컬레이션 3칸 | ⏳ **마찰이 재료** | 상상으로 만들면 **일곱 번째 하네스** |
