...
| **R5-12** | claim table 이 **`verified` 문서에서만 검사된다** — `draft` 는 무검증 | 검사 범위 확대 (기존 위반 먼저 집계) |
| **R5-14** 🆕 | claim 검사가 **굵게 쓴 ID 행을 통째로 건너뛴다** — **실측 42행**, 배치 1~4 의 claim 은 **한 줄도 검사된 적이 없다** | 굵게 해제 시 **10행 실패**(집계 완료). R5-12 와 독립 |
