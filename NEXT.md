...
> 🔴 **덤으로 같은 결함을 하나 더 잡았다.** 이 계기도 `commit_id` 를 쓰고 있었다 —
> **GitHub 이 그 값을 head 로 옮기므로 대리지표가 통째로 어긋난다.** `original_commit_id` 로 고쳤다
> (`touched_after` 47% → **44%**). **벽에서 잡은 것과 같은 뿌리인데 도구가 둘이었다** —
> 한 곳을 고칠 때 **같은 API 를 읽는 다른 곳**을 안 봤다.
