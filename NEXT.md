...
`tools/check_review_signal.py` — **네트워크(CI 밖)**. 첫 눈금: **리뷰된 PR 8 · finding 17 · P1 9 · P2 8 · P0 0 · PR당 2.1**.
...
`divcal` **v2.12.1** · 검사 **14종** · 파이썬 시험 **215** · 셸 시험 **13** · 열린 격차 **5**
...
🔴 **격차 5건 중 넷은 *지금 못 하는* 것이다** — 시간·외부 접근·소유자 행동을 기다린다:

| | 왜 |
|---|---|
| `R5-24` | 🔄 **선결 조건 해소 2026-08-29** — 핀 장치는 **있다**(마켓 항목의 `sha` · 공식 마켓 238/291). 남은 건 *무엇을 어느 범위로 켜나* — 소유자 판단 |
| `R5-40` | 🔄 **착수분은 끝났다** — 네 측면을 읽고 규칙 하나를 승격했다. **남은 ⓑ·ⓒ 가 `R5-24` 와 만들 것 ⑥ 에 걸려 있다** |
| `R5-26` | **의도적 대기** — 상상으로 만들면 일곱 번째 하네스 |
| `R5-27` | 1차 출처 둘이 **403 · 미특정** |
| `R5-37` | **2주 관측**(2026-09-11 이후) — 계기는 달렸고 눈금이 4/4 로 움직였다 |

### 🔴 남은 미검증 — **실제 과제 하나면 대부분 풀린다**

| | 무엇 | 왜 아직 못 쟀나 |
|---|---|---|
| ② | `ponytail`(최소주의) × `taste-skill`(디자인 풍부) **충돌** | **프론트엔드 과제가 없다** |
| ③ | 🔴 **스킬이 실제로 뜨나** | 🔵 **커맨드는 뜬다**(2026-08-30 확인). 남은 건 `SKILL.md` 형태. **한 번도 안 쟀다.** 설계 전체가 *"안내 층이 발화한다"* 는 가정 위에 서 있고, 원칙 03 이 스스로 *"보여주는 것에는 미검증"* 이라 적어놨다. **goppi 가 죽은 방식이 이 형태다** |

| ④ | **배포 축 ⓓⓔⓕ** — 남이 설치해서 서나 | **첫 외부 설치자**가 있어야. 혼자 재면 캐시가 가리고, 자기가 만든 거라 안 잊는다 |
| 🆕 ⑤ | **시간축 ⓖⓗ** — 낸 것이 **한 달 뒤에도 살아 있나** | 🔴 **유지할 프로젝트가 없다.** `divcal` 은 동결이다 — **우리는 무언가를 유지해본 적이 없다** |

(미검증 ①(마켓 의존성)은 **2026-08-29 에 해소** — 첫 설치가 실패했고 고쳤다. 기록은 `PLUGIN-DESIGN`.)

🔵 **④⑤ 는 2026-08-30 에 *사전 등록* 됐다** — 관측 전에 박아야 하기 때문이다(`04` §판정 기준).
⚠️ **⑤ 가 늦게 발견된 이유가 기록돼 있다**: 우리 지표가 전부 ***"낼 때"*** 를 쟀고,
**완료의 정의가 *end-to-end 한 번 통과*** 라서 **낸 뒤를 볼 자리가 없었다.**

### 그래서 다음은 셋 중 하나다

| | 무엇 | 누가 |
|---|---|---|
| **가** | 🎯 **진짜 새 프로젝트 하나** — `/kickoff` 이 이제 플러그인으로 깔려 있다. **②③이 동시에 풀리고** 목적의 *"반복해서"* 도 채운다 | 아이디어는 **소유자** |
| 나 | `R5-24` — **무엇을 어느 범위로 켜나**(핀은 풀렸다) | **소유자 판단** |
| 다 | 남은 만들 것 ⑧⑨⑩⑬ — ⑩ 은 [재판정](direction/04-the-plan.md)돼 크기가 줄었다 | 에이전트 |

🔴 **가를 권한다.** `standards` 안에서 혼자 할 수 있는 것은 거의 다 했고,
**남은 미검증은 전부 *실사용* 에서만 풀린다.**
