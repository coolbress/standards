...
| **R5-18** 🆕 **스캐폴드가 프로젝트 성격에 유연하지 않다** (2026-08-26) | 새 프로젝트를 뜨면 라이선스·공개여부가 **고정으로 복사된다**. `new-project.sh` 는 **`--public` 하드코딩**, `project-template` 은 **`LICENSE`(MIT)를 그대로 복사**, `/kickoff` 는 아키타입 **2문항**(공개·개인정보)만 묻는다 | 🔴 **`direction/05` 자신의 판정 기준을 적용하면 라이선스는 물어야 하는 쪽이다** — 그 기준은 *"의도는 물어야 한다 — 공개 여부와 개인정보는 **파일에 안 적혀 있고 둘 다 법적 의무를 부른다. 틀리면 되돌릴 수 없다**"* 인데, **라이선스가 정확히 그 범주다**(의도 · 법적 효력 · 공개 배포 후 소급 철회 어려움). **기준은 있는데 적용이 안 됐다.** ✅ **반면 릴리스·CODEOWNERS 는 묻지 않는 것이 맞다** — 릴리스는 나중에 태그 하나면 되고(가역), CODEOWNERS 는 **저장소가 드러낸다**(기여자 수) | ① `/kickoff` 아키타입 문항 **2 → 3**(라이선스) ② `new-project.sh` 의 `--public` 과 템플릿 `LICENSE` 를 그 답으로 채운다. ⚠️ **P40 경계 유지** — 문진표를 만들지 않는다. **되돌릴 수 없고 파일로 알 수 없는 것만** 묻는다 |
| **R5-19** 🆕 **CHANGELOG 가 릴리스 결정과 어긋난다** (2026-08-26) | 바닥이 CHANGELOG 를 **무조건 MUST** 로 두고 템플릿도 넣어주는데, 같은 날 **릴리스는 `workflows` 만 한다**고 확정했다 | 🔴 ***Keep a Changelog* 는 릴리스 단위로 항목을 쌓는 형식이다** — **릴리스를 하지 않는 저장소에는 채울 단위가 없다.** census 도 **52/51%** 로 동전 던지기다. 지금 릴리스를 안 하는 두 저장소에 **빈 CHANGELOG 가 서 있다** — `presence≠adequacy`(C50-14)의 실례를 이 프로젝트가 만들고 있는 셈 | ✅ **바닥에서 릴리스 묶음과 함께 조건부로 표시했다**(2026-08-26). ⬜ 템플릿의 `CHANGELOG.md` 를 어떻게 할지는 R5-18 ②와 함께 처리 |
