...
| **R5-15** 🆕 **테스트 파일 4개가 있는데 CI 는 하나도 돌리지 않는다** (2026-08-25 발견) | `.github/workflows/ci.yml` 의 `integrity` 잡이 돌리는 것은 `validate_corpus.py` · `build-routes --check` · `evaluate_retrieval_contract.py` **3개뿐**이고, `tools/test_*.py` **4개는 어디서도 실행되지 않는다** | 🔴 **실측 2026-08-25**: `python3 -m unittest discover tools/` → **26개 중 7개 실패.** 전부 `test_trustworthy_completion_contract.py` 이고 원인은 하나 — **`interpretation/foundation/` 경로가 사라졌는데 테스트가 아직 그것을 읽는다**(`trustworthy-completion-evaluation-protocol.md` · `goppi-worth-hypothesis.md`). **디렉터리 개편 이후 계속 깨져 있었고 아무도 몰랐다 — 돌리지 않으니까.** R5-12·R5-14 와 **같은 계열의 세 번째 사각지대**다: *검사가 존재하는데 실행되지 않는다* | ① 7건이 가리키는 문서가 **어디로 갔는지 먼저 확정**한다(옮겨졌나 · 폐기됐나). 폐기면 테스트도 폐기, 이동이면 경로 갱신. ② 그 다음 CI 에 `python3 -m unittest discover -s tools -p 'test_*.py'` 를 **추가**한다. ⚠️ **순서를 바꾸면 안 된다** — 지금 붙이면 `integrity` 가 빨간불이 되고, 이 저장소는 빨간 상태에서 아무것도 머지할 수 없다(벽이 자기 발을 묶는다) |
