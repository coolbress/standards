...
| ~~**R5-18**~~ ✅ **종료 2026-08-26** 스캐폴드가 프로젝트 성격에 유연하지 않다 | 새 프로젝트를 뜨면 라이선스·공개여부가 **고정으로 복사된다**. `new-project.sh` 는 **`--public` 하드코딩**, `project-template` 은 **`LICENSE`(MIT)를 그대로 복사**, `/kickoff` 는 아키타입 **2문항**(공개·개인정보)만 묻는다 | 🔴 **`direction/05` 자신의 판정 기준을 적용하면 라이선스는 물어야 하는 쪽이다** — 그 기준은 *"의도는 물어야 한다 — 공개 여부와 개인정보는 **파일에 안 적혀 있고 둘 다 법적 의무를 부른다. 틀리면 되돌릴 수 없다**"* 인데, **라이선스가 정확히 그 범주다**(의도 · 법적 효력 · 공개 배포 후 소급 철회 어려움). **기준은 있는데 적용이 안 됐다.** ✅ **반면 릴리스·CODEOWNERS 는 묻지 않는 것이 맞다** — 릴리스는 나중에 태그 하나면 되고(가역), CODEOWNERS 는 **저장소가 드러낸다**(기여자 수) | ✅ **완료 2026-08-26** — ① `/kickoff` 아키타입 문항 **2 → 3**(라이선스 추가) ② `new-project.sh` 가 `--private` · `--license=<spdx>` 를 받는다(로직 **9→19줄**, 원칙 02 의 20줄 신호 안). 🔴 **비공개는 답을 믿지 않고 GitHub 이 답하게 한다** — 토큰에 `user` 스코프가 없어 플랜을 미리 못 읽는다(`plan: null`). 그래서 **룰셋을 실제로 걸어보고, 실패하면 방금 만든 원격 저장소를 지우고 멈춘다** — **벽 없는 저장소를 남기지 않는다.** 안내는 GitHub 자신의 문구를 인용한다(*"Upgrade to GitHub Pro or make this repository public"*)이고 길은 둘뿐임을 명시한다. ⚠️ **순서**: 라이선스 교체는 **룰셋을 걸기 전에** 한다 — 건 뒤엔 `main` 직접 푸시가 막힌다. 본문은 **GitHub 공식 `/licenses/<spdx>`**. ✅ **P40 경계 유지** — 문진표를 만들지 않았다(3문항). 릴리스·CODEOWNERS 는 **가역이거나 저장소가 드러내므로 묻지 않는다** |
