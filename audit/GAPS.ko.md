...
| **R5-14** 🆕 claim table 검사가 **굵게 쓴 claim ID 행을 통째로 건너뛴다** (2026-08-25 배치 5 발견) | `validate_corpus.py` 의 `CLAIM_ID_RE = ^[A-Z][A-Z0-9-]+-\d{3}$` 가 **셀 내용 전체와 완전 일치**를 요구하는데, 재검증 배치들이 ID 를 `**IPW-001**` 처럼 굵게 써 왔다. **`**` 때문에 정규식이 안 맞아 그 행은 claim 행으로 인식조차 되지 않는다** | 🔴 **실측 2026-08-25: 6개 문서 42행이 이렇게 건너뛰어졌다** — `28 overview` **IPW-001~018 전부(18행)** · `codereview--facts` CR-001~008(8행) · `elicitation…` ELI 6행 · `multi-agent…` MAO 6행 · `testing--facts` TDD 2행 · `17 overview` REL 2행. **즉 배치 1~4 가 만든 claim 행은 단 한 줄도 검사된 적이 없다.** R5-12(`verified` 문서에서만 검사)와 **독립된 두 번째 사각지대**이고, R5-12 를 고쳐도 이건 남는다 | 굵게를 벗기면 검사가 켜진다. **켜기 전에 위반을 셌다(R5-12 의 지시): 42행 중 10행이 실패한다** — `codereview--facts` CR-001~008 **8행**(Evidence 가 등록 source ID 가 아니라 원시 URL·산문) · `28 overview` **IPW-016·017 2행**(*"(인용 부적합 판정)"* 등 산문). 나머지 32행은 그대로 통과한다. <br>✅ **배치 5 자기 몫은 처리** — MAO 6 · ELI 6 을 **굵게 없이** 넣어 **검사되는 상태로** 두었고 0 errors 다. 나머지 30행의 굵게 해제 + 10행 수정은 **별건**으로 남긴다(한 번에 켜면 빨간불이 된다는 R5-12 의 경고 그대로) |
