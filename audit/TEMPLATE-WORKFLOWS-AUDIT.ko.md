...
**규칙 존재 여부·strict·머지 방법·대상 브랜치**까지 본다 |
...
### 🔒 A-1 이후 — **관리자 열쇠는 이 컴퓨터에 없다** (2026-08-27)

| | 누가 |
|---|---|
| 코드 · 이슈 · PR · 워크플로 **파일** | 에이전트 |
| 룰셋 · Actions 정책 · 시크릿 · Environments · **CodeQL default setup** · 저장소 생성 | **사람** — 토큰을 **그때그때 붙여넣는다** |

🔴 **처음 판은 규율이었지 벽이 아니었다.** *"에이전트는 `env -u GH_TOKEN` 을 쓰지 않는다"* 로 막으려 했는데,
검증해 보니 **`gh` 를 안 거쳐도 뚫렸다**:

```
security find-generic-password -s 'gh:github.com' -w
→ 🔴 프롬프트 없이 토큰 값이 그대로 나왔다
```
...
| | 전 | 후 |
|---|---|---|
| `security … -w` 로 토큰 꺼내기 | 🔴 **프롬프트 없이 꺼내짐** | ✅ **항목 자체가 없다** |
| `env -u GH_TOKEN gh auth status` | 🔴 관리자로 로그인됨 | ✅ **`not logged into any GitHub hosts`** |
| `env -u GH_TOKEN gh api …/rulesets` | 🔴 읽힘 | ✅ **인증 실패** |
| 에이전트 평소 작업(읽기·PR·`git push`) | 정상 | **정상** |

⚠️ **완전한 경계는 아니다 — 정직하게 적는다.** 붙여넣은 몇 초 동안은 같은 OS 사용자다.
**진짜 기술적 경계는 별도 OS 사용자나 컨테이너**뿐이고, 그건 *"세팅에 시간 쓰기 ✕ · 상한 반나절"* 에 걸려
**의도적으로 안 했다.** 지금 상태는 ***"평소에 물건이 없고, 있는 몇 초 동안은 사람이 앞에 있다"*** 다.
