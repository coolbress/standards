...
- **Branching = trunk-based / GitHub Flow.** One long-lived `main`; all work on **short-lived feature branches** (hours, not weeks) merged back continuously. DORA's measured rule: **≤3 active branches**, **merge to trunk ≥ once/day**, **no code freezes / integration phases** — these correlate with elite delivery performance. [lit] GitHub Flow is the lightweight default; heavyweight GitFlow (long release/develop branches) is now an anti-pattern for continuously-deployed software. [inferred]
