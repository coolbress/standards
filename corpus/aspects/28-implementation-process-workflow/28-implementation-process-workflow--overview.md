...
| IPW-017 | synthesis | ⚠️ **재시도 상한·진동 감지는 구현 사례는 있으나 값이 확립되지 않았다.** 문제의 실재는 확인된다(IAL-Scan: **6,549 저장소** 정적 분석 → 후보 74건 중 수동 확인 **68건 · 47개 프로젝트 · 정밀도 91.9%**). 구현도 있다(OpenAI Agents SDK 의 `max_turns` → `MaxTurnsExceeded`; A→B→A 실패 서명 감지 사례). **그러나 최적 상한값이나 보편적 진동 탐지법은 확립돼 있지 않다** — *"상한을 두라"* 까지가 근거이고 **얼마로 두는지는 판단이다** | `IAL-SCAN-2026`; `OPENAI-AGENTS-SDK-RUNNING` | medium | 2026-08-25 **한정 추가** |
