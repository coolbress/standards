...
| IPW-021 | empirical | 🎯 **리뷰 finding 을 *실행* 으로 판정하는 벤치마크가 나왔다.** c-CRAB 은 사람의 리뷰 코멘트를 **실행 가능한 시험**으로 변환해 *버그 코드에서 실패하고 고치면 통과하는지* 를 오라클로 쓴다 — 텍스트 유사도를 버렸다. 통과율: **사람 100% · Claude Code 32.1% · Devin 24.8% · PR-Agent 23.1% · Codex 20.1%**, **넷을 합쳐도 41.5%**(234 중 97). ⭐ **그런데 수동 검수에서 코멘트의 84%가 유용했다** — 저자는 격차를 *소음* 이 아니라 **우선순위 차이**로 읽고, 정렬을 높일 **최강 지렛대로 저장소 고유 맥락(문서·컨벤션)** 을 지목한다. 🔴 위양성 감소 *기법* 은 이 논문이 제시하지 않는다. ⚠️ **동료심사 전 프리프린트** | `CODE-REVIEW-AGENT-BENCHMARK-2026` | medium-high | 2026-09-02 **신규** |
