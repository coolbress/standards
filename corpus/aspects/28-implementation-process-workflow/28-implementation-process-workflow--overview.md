...
| IPW-007 | synthesis | ⚠️ **그러나 절대 성능은 낮다.** 최선 조건(CCR)에서도 **F1 28.6%** 다. *"+16%"* 는 **상대 개선**(28.6/24.6)이지 절대 수준이 아니다. → **독립 리뷰어는 갖출 값이 있지만 안전망으로 취급하면 안 된다.** 게이트는 여전히 CI 다. 🔴 **정정 2026-09-01 — 이 줄은 *"오류의 약 71%를 놓친다"* 고 적고 있었다. 틀린 유도다.** F1 은 정밀도와 재현율의 **조화평균**이라 `1 − F1` 은 위음성률이 **아니다** (반례: 정밀도 20% · 재현율 50% → F1 28.6% 인데 놓친 것은 50%). **원문은 recall 을 따로 보고하지 않는다**(1차 재확인 2026-09-01) — 그래서 **놓친 비율은 우리가 모른다.** 🔬 이 오류를 잡은 것은 `third-party / review` 의 **제3자 리뷰 첫 실전**이다 | `CROSS-CONTEXT-REVIEW-2026` | high | 2026-09-01 **정정** |
...
| IPW-019 | empirical | 🔴 **AI 가 낸 PR 은 대부분 아무도 안 본다.** AIDev · 별 100+ 저장소의 **AI 생성 PR 33,596건** 중 **61.38%가 리뷰 활동 기록 자체가 없다.** 리뷰된 38.62% 안에서도 **58.77%는 에이전트끼리만** 봤고 사람만 본 것은 **10.14%**다. 리뷰 댓글 39,122건 중 **71.58%가 에이전트 작성**. ⚠️ 저자가 명시한다 — *"we do not assess code quality or review effectiveness"* (품질이 아니라 **존재 여부**를 잰 것이다) | `AI-PR-HUMAN-REVIEW-2026` | high | 2026-09-01 **신규** |
| IPW-020 | empirical | 🔴 **리뷰 에이전트의 신호는 대체로 소음이다.** PR **3,109건** · 리뷰 기록 **19,450건** · CRA **13종**. signal ratio(= Tier1·2 댓글 ÷ 전체)가 **13종 중 12종(92.31%)에서 60% 미만** · 닫힌 CRA-only PR 의 **60.2%가 0~30% 신호대**. 머지율도 **CRA-only 45.20% vs 사람만 68.37%** (23.17%p · χ²=83.03 · p<0.001). ⭐ 저자 권고가 **설계에 바로 쓰인다**: ① **범용 대신 좁고 구체적인 검사** ② **머지 전 사람 승인** ③ **CRA 댓글은 개발자의 직접 대응이 아니라 *사람 리뷰를 촉발* 하게 하라.** ⚠️ *처리율 0.9~19.2%* 는 이 논문이 Gan et al.(2025)을 **인용**한 2차 수치다 — 쓰려면 원문을 봐야 한다 | `CODE-REVIEW-AGENTS-EMPIRICAL-2026` | high | 2026-09-01 **신규** |
| IPW-021 | empirical | 🎯 **리뷰 finding 을 *실행* 으로 판정하는 벤치마크가 나왔다.** c-CRAB 은 사람의 리뷰 코멘트를 **실행 가능한 시험**으로 변환해 *버그 코드에서 실패하고 고치면 통과하는지* 를 오라클로 쓴다 — 텍스트 유사도를 버렸다. 통과율: **사람 100% · Claude Code 32.1% · Devin 24.8% · PR-Agent 23.1% · Codex 20.1%**, **넷을 합쳐도 41.5%**(234 중 97). 🔴 **사람의 100% 는 경쟁 점수가 아니라 *구성상 천장* 이다** — 시험이 **바로 그 사람 코멘트에서** 만들어졌다(*"For each instance, we ask a coding agent to improve the code using the human review comment as guidance"*). **«사람이 모델보다 낫다» 로 읽으면 원자료를 잘못 읽는 것이다.** 저자가 직접 막는다: *"The lower pass rates do not necessarily imply that the generated reviews are low quality and not useful. Instead, they suggest that the reviews produced by automated tools are less aligned with the types of issues that human reviewers typically raise."* ⭐ **그런데 수동 검수에서 코멘트의 84%가 유용했다** — 저자는 격차를 *소음* 이 아니라 **우선순위 차이**로 읽고, 정렬을 높일 **최강 지렛대로 저장소 고유 맥락(문서·컨벤션)** 을 지목한다. 🔴 위양성 감소 *기법* 은 이 논문이 제시하지 않는다. ⚠️ **동료심사 전 프리프린트** | `CODE-REVIEW-AGENT-BENCHMARK-2026` | medium-high | 2026-09-02 **신규** |
| IPW-022 | empirical | 🔴 **LLM 리뷰어는 맞는 코드를 틀렸다고 하는 쪽으로 치우친다**(체계적 과잉교정). 반직관적 관찰: *"more detailed prompt design, particularly with those requiring **explanations and proposed corrections**, leads to higher misjudgment rates"*. 처방은 **Fix-guided Verification Filter** — 모델이 낸 **수정안을 실행 가능한 반사실 증거**로 취급해 원본과 수정본을 시험으로 돌려 판정한다. ⚠️ **수치는 하나도 인용하지 않는다** — 위양성률·모델 수·표본·개선폭이 **초록에 없고 본문을 안 읽었다**(2026-09-02). **동료심사 전 프리프린트** | `LLM-REVIEWER-OVERCORRECTION-2026` | medium (기제만 · 수치 미확인) | 2026-09-02 **신규** |
| IPW-023 | empirical | 🎯 **교차 모델 리뷰는 *방향* 이 결과를 뒤집는다 — 통제실험.** LeetCode hard·medium **116과제** · Claude·Codex **여섯 조건**. **Codex 단독 71.6% → Claude 가 리뷰 89.7%**(p=.001) · **Codex 자기리뷰 84.5%**(p=.022) · **Claude 단독 91.4% → Codex 가 리뷰 82.8%**(p=.046 — 🔴 **악화**) · **Claude 자기리뷰 91.4%**(**변화 없음**). 저자: *"use Claude to review Codex, not the other way around."* 🔴 **두 갈래가 우리에게 직접 닿는다**: ⓐ **강한 모델의 자기리뷰는 이득이 0 이다** — *«내가 나를 리뷰한다»* 는 층은 값이 없다 · ⓑ **약한 리뷰어가 강한 초안을 보면 통과율이 떨어진다** — `IPW-022` 의 **과잉교정**과 같은 현상을 다른 각도로 잰 것이다. 🔬 **결정적 조건**: *"Reviewers could see the problem and draft but **couldn't execute tests**"* — **의견만 냈고 실행이 없었다.** 처방이 `IPW-021`·`IPW-022` 와 같은 곳을 가리킨다. ⚠️ **동료심사 전 프리프린트** · **과제가 LeetCode 정답성이라** 문서·리서치 저장소나 fail-open 탐지에는 **직접 적용 못 한다** | `CROSS-MODEL-REVIEW-2026` | medium | 2026-09-02 **신규** |
