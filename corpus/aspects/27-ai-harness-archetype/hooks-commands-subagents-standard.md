...
| **Breadth-first, parallelizable** investigation across independent directions, where value > token cost | **Multiple subagents (orchestrator-worker)** | +90.2% on research eval; but ~15× tokens → only when parallel and high-value [lit, empirical]. |
