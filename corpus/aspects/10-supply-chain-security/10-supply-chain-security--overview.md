...
- **Pin dependencies (and the build).** Lockfiles (`pnpm-lock.yaml`/`uv.lock`/`Cargo.lock`) pin the *resolved* graph by content hash; runtime versions pinned (`.nvmrc`/`packageManager`/`.python-version`). This makes builds reproducible and blocks silent transitive substitution. [lit, normative] NIST SSDF PS-3 (archive/protect each release), [lit, normative] SLSA Build L2+ (verified provenance over a controlled build).
...
- [lit, normative] **OpenSSF Baseline (OSPS)** — tiered minimum security requirements (~41 controls × 3 maturity levels × 6 lifecycle categories) for OSS projects; the leveling (L1 floor → L3 widely-adopted) is the named grounding for context-conditioned deferral. https://baseline.openssf.org/ · version doc https://baseline.openssf.org/versions/2025-02-25.html
