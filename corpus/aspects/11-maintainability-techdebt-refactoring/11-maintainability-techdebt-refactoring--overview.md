...
- **Deprecate and dispose on a schedule, don't accrete.** Mark with `@deprecated` + a replacement pointer + a removal version; honour a deprecation window under semver; then actually *delete* dead code/feature flags/old branches. SWEBOK names *retirement/disposal* as a real lifecycle stage — unused code is negative-value maintenance surface. [lit, normative]

## Evidence (lit + census)

- **SWEBOK Guide v4.0** (IEEE CS, 2024) — KA *Software Maintenance*: maintenance categories (adaptive/perfective/corrective/preventive), maintainability as designed-in cost-of-change, retirement/disposal stage. [lit, normative] https://www.computer.org/education/bodies-of-knowledge/software-engineering
