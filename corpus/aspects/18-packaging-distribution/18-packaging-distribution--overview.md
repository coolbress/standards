...
- **Container / Helm publish (backend/service/monorepo).** Build + push an immutable, digest-pinned image to a registry (GHCR/ECR/Docker Hub) from CI; ship a versioned Helm chart for k8s consumers. `[census]` container 53/62% (backend **79%**), cd_deploy 64/68%.
...
- **Changelog is the human-facing distribution artifact.** Keep-a-Changelog `CHANGELOG.md` + Conventional-Commits history feeds auto-generated notes & SemVer bumps. `[census]` changelog 55/52%; CC-adopted (≥30% of commits) **45/67%** — the fastest-rising ④ signal (+22 weighted).
