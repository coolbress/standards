#!/usr/bin/env bash
# Skeleton census of top OSS coding harnesses. Reads census_list.txt (one full_name/line),
# fetches repo meta + recursive git tree via gh api, emits file-presence booleans to results.tsv.
set -uo pipefail
cd "$(dirname "$0")"
LIST=census_list.txt
OUT=results.tsv
ERR=errors.txt
TMP=$(mktemp)
: > "$ERR"
