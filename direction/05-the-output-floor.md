...
| **12** 🔶 | **SAST + 시크릿 탐지를 `coolbress/workflows` 에** — **엔진은 아직 정하지 않았다** | ✅ **제약은 확인됐다**: 비공개 저장소의 CodeQL 은 **`GitHub Code Security` 라이선스가 필요하다**(제품명이 GHAS 에서 바뀌었다 · `FFA-008`). **Pro 만으로는 안 된다.** 🔴 **하지만 *"그러니 Semgrep"* 은 근거가 아니라 소거법이다** — **탐지율·오탐 비교는 미조사**([`AUDIT`](../audit/TEMPLATE-WORKFLOWS-AUDIT.ko.md) **C-2**). 🔴 **gitleaks 도 마찬가지** — push protection 이 이미 켜져 있는데 **겹침이 값을 하는지 재본 적 없다**(**C-3**) |
